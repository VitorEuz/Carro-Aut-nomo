...
# Função para filtro cinza
def apply_gray_filter(frame, intensity):
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    gray_bgr = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
    blended = cv2.addWeighted(frame, 1 - intensity / 100, gray_bgr, intensity / 100, 0)
    return blended
...
cv2.createTrackbar("Light Reduction", "Controles", 0, 100, nothing)
...
# ------------------------------CODIGO 2---------------------------------------------------------------------
    ...
    # Obter a intensidade de redução de luz (de 0 a 100) pela barra de controle
    light_reduction = cv2.getTrackbarPos("Light Reduction", "Controles") / 100

    # Aplicar o filtro para diminuir a intensidade da luz
    frame2 = frame2*(1 - light_reduction)
    frame2 = np.clip(frame2, 0, 255).astype(np.uint8)  # Garantir que os valores fiquem no intervalo correto
        ...
        for box in boxes:
            ...
            if label in KNOWN_WIDTHS:  # Verifica se a classe é uma das desejadas
                ...
                if label == "traffic light":
                    region_upper = frame2[y1:int(y1 + h // 3), x1:x2]
                    region_lower = frame2[int(y1 + 2 * h // 3):y2, x1:x2]

                    # Converte a região superior para o espaço de cores HSV
                    hsv_upper = cv2.cvtColor(region_upper, cv2.COLOR_BGR2HSV)

                    # Define os intervalos para a cor vermelha no espaço HSV
                    lower_red = np.array([0, 100, 100])
                    upper_red = np.array([10, 255, 255])

                    # Mascara para detectar vermelho na região
                    mask_red = cv2.inRange(hsv_upper, lower_red, upper_red)

                    # Verifica se há vermelho suficiente na região
                    ...
                    # Verifica a distância e o status do semáforo
                    if light_status == "Red light" and distance < 5:
                        ParaSemaforo = 1
